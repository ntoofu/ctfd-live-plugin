...
from CTFd.plugins import register_plugin_asset, override_template
...
class ChallengeViews(db.Model):
    ...
    challenge_id = db.Column(db.Integer, db.ForeignKey('challenges.id', onupdate="CASCADE", ondelete="CASCADE"))
...
live_namespace = Namespace(
    "challenges", description="Endpoint used by 'live' plugin"
)
...
class LiveChallengeViews(Resource):
    ...
    @during_ctf_time_only
    def get(self):
        subquery = db.session.query(ChallengeViews,
                                    func.rank().over(
                                        order_by=ChallengeViews.date.desc(),
                                        partition_by=ChallengeViews.user_id).label('order')
                                    ).subquery()
        views = db.session.query(subquery, Users.team_id, Users.name).filter(subquery.c.order==1).join(Users, subquery.c.user_id == Users.id)
        return {
                "success": True,
                "data": [
                    {"user_id": v.user_id,
...
def send_message(user_id, challenge_id, msg):
    ...
    current_app.events_manager.publish(data={"user_id": user_id, "challenge_id": challenge_id, "message": msg}, type="live")
    return
...
def load(app):
    ...
    override_template('live.html', '{% extends "base.html" %}{% block scripts %}<script type="text/javascript" src="/plugins/live/assets/main.js"></script><link rel="stylesheet" type="text/css" href="/plugins/live/assets/main.css">{% endblock %}{% block content %}<h1>Live Scoreboard</h1>{{content}}<div id="ctfd-live-plugin-container"></div>{% endblock %}')
